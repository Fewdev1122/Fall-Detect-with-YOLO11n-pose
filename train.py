...
print(f"Original Train counts: {np.bincount(y_train)}") # ดูจำนวนก่อนทำ
smote = SMOTE(random_state=42)
X_train_resampled, y_train_resampled = smote.fit_resample(X_train, y_train)

print(f"Resampled Train counts: {np.bincount(y_train_resampled)}") # ดูจำนวนหลังทำ (จะเท่ากันแล้ว)
